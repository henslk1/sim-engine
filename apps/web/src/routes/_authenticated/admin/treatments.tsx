...
function TreatmentsPage() {
  ...
  function toggleExpand(treatmentId: string) {
    setExpandedTreatmentId((prev) => (prev === treatmentId ? null : treatmentId))
    setEditingItemId(null)
    setEditingItem(null)
    setNewItem(emptyItem())
  }
          ...
          <section className="rounded-lg border border-border bg-card shadow-sm">
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                  ...
                  <Fragment key={t.id}>
                    <tr className={`border-b border-border ${expandedTreatmentId === t.id ? "bg-muted/30" : ""}`}>
                      ...
                      <td className="px-4 py-2 text-right space-x-1">
                        <Button size="sm" variant="ghost" onClick={() => toggleExpand(t.id)} className="text-xs">
                          {expandedTreatmentId === t.id ? "▲" : "▼"} Items
                        </Button>
                ...
                {treatments?.length === 0 && (
                  <tr>
                    <td colSpan={5} className="px-4 py-6 text-center text-sm text-muted-foreground">No treatments for this condition yet.</td>
                  </tr>
                )}
              </tbody>
            </table>
