...
function CareActionsPage() {
  ...
  const saveLongTerm = trpc.admin.care.saveLongTerm.useMutation({
    onSuccess: () => {
      ...
      setEditingLongTerm(emptyLongTerm())
    },
  })
  ...
  const [editingLongTerm, setEditingLongTerm] = useState<LongTermRow>(emptyLongTerm())
  ...
  function submitLongTerm() {
    if (!gameId || !editingLongTerm.name.trim() || !editingLongTerm.intervalCycles) return
    saveLongTerm.mutate({
        ...
        <div className="grid grid-cols-[300px_1fr] gap-2 items-start">
          <section className="rounded-lg border border-border bg-card shadow-sm">
            <div className="border-b border-border bg-secondary/40 px-3 py-2">
              <h2 className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground">Care Action Details</h2>
            </div>
            <div className="p-3 space-y-3">
              ...
              <div className="flex flex-col gap-1">
                <label className="text-[10px] font-medium uppercase tracking-wide text-muted-foreground">Cost Type</label>
                <select
              ...
              {editing.costType === "CURRENCY" && (
                <div className="flex flex-col gap-1">
                  <label className="text-[10px] font-medium uppercase tracking-wide text-muted-foreground">Currency Amount</label>
                  <Input
                    className="h-8 text-sm"
                    type="number"
                    min="0"
              ...
              <div className="flex flex-col gap-1">
                <label className="text-[10px] font-medium uppercase tracking-wide text-muted-foreground">Care Score Gain</label>
                <Input
                  className="h-8 text-sm"
                  type="number"
                  step="0.01"
              ...
              <div className="flex flex-col gap-1">
                <label className="text-[10px] font-medium uppercase tracking-wide text-muted-foreground">Energy Restore</label>
                <Input
                  className="h-8 text-sm"
                  type="number"
                  step="0.01"
              ...
              <div className="flex flex-col gap-1">
                <label className="text-[10px] font-medium uppercase tracking-wide text-muted-foreground">Mood Boost</label>
                <Input
                  className="h-8 text-sm"
                  type="number"
                  step="0.01"
                  ...
                  <tbody>
                    {actionItems?.map((item: NonNullable<typeof actionItems>[number]) =>
                      editingItemId === item.id ? (
                        ...
                        <tr key={item.id} className="border-b border-border last:border-0">
                          <td className="px-3 py-2 font-medium text-foreground">{item.itemDef.name}</td>
    ...
    <div className="p-4 space-y-3 max-w-4xl mx-auto">
      <div>
        <div className="flex items-center justify-between mb-4">
          <h1 className="font-serif text-xl font-semibold text-foreground">Care Actions</h1>
          <Button onClick={() => setEditing(emptyCareAction())}>+ New Care Action</Button>
        </div>
        <div className="rounded-xl border border-border bg-card shadow-md p-2">
        <section className="rounded-lg border border-border bg-card shadow-sm">
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-border">
                ...
                <th className="px-3 py-2 text-left text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">Cost Type</th>
                <th className="px-3 py-2 text-left text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">Score Gain</th>
            ...
            <tbody>
              {actions?.map((a) => (
                <tr key={a.id} className="border-b border-border last:border-0">
                  ...
                  <td className="px-3 py-2 text-muted-foreground">{a.costType}</td>
                  <td className="px-3 py-2 text-muted-foreground">{a.careScoreGain}</td>
                  <td className="px-3 py-2 text-center text-muted-foreground">{a._count.items}</td>
              ...
              {actions?.length === 0 && (
                <tr>
                  <td colSpan={5} className="px-3 py-6 text-center text-sm text-muted-foreground">No care actions yet.</td>
                </tr>
              )}
            </tbody>
          </table>
      ...
      <div className="rounded-xl border border-border bg-card shadow-md p-2">
      <div className="grid grid-cols-[1fr_300px] gap-2 items-start">
        <section className="rounded-lg border border-border bg-card shadow-sm">
          <div className="border-b border-border bg-secondary/40 px-3 py-2">
            <h2 className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground">Long-term Care Actions</h2>
          </div>
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-border">
        ...
        <section className="rounded-lg border border-border bg-card shadow-sm">
          ...
          <div className="p-3 space-y-3">
            ...
            <div className="flex flex-col gap-1">
              <label className="text-[10px] font-medium uppercase tracking-wide text-muted-foreground">Interval (cycles)</label>
              <Input
                className="h-8 text-sm"
                type="number"
                min="1"
            ...
            <div className="flex flex-col gap-1">
              <label className="text-[10px] font-medium uppercase tracking-wide text-muted-foreground">Grace Period (cycles)</label>
              <Input
                className="h-8 text-sm"
                type="number"
                min="0"
            ...
            <div className="flex gap-2 pt-1">
              ...
              {editingLongTermId && (
                <Button variant="ghost" onClick={() => { setEditingLongTermId(null); setEditingLongTerm(emptyLongTerm()) }}>
                  Cancel
                </Button>
              )}
            </div>
