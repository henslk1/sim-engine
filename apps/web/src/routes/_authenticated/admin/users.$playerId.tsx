...
export const Route = createFileRoute("/_authenticated/admin/users/$playerId")({
...
function PlayerDetail() {
  ...
  const assignRoleMutation = trpc.admin.ops.players.assignRole.useMutation({ onSuccess: () => refetch() })
  const removeRoleMutation = trpc.admin.ops.players.removeRole.useMutation({ onSuccess: () => refetch() })
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          ...
          <ActionCard title="Staff Role">
            {staffRoles.length > 0 ? (
              ...
              <p className="text-sm text-muted-foreground">No staff roles.</p>
            )}
            <div className="flex gap-2 pt-1">
              <select
                value={roleToAssign}
                onChange={e => setRoleToAssign(e.target.value as typeof roleToAssign)}
                className="flex-1 rounded-md border border-border bg-background px-3 py-2 text-sm outline-none focus:border-primary"
              >
